execute as @e[type=area_effect_cloud,tag=CustomMissileArea,sort=nearest,limit=1] at @s run fill ^-8 ^6 ^9 ^8 ^8 ^11 minecraft:air
...
execute at @e[type=area_effect_cloud,tag=CustomMissileArea,sort=nearest,limit=1] run particle minecraft:explosion_emitter ^-1 ^8 ^10 0 0 0 100 7