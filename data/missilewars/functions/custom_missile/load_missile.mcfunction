...
execute as @e[type=area_effect_cloud,tag=CustomMissileArea,sort=nearest,limit=1] at @s store result score Temp m.Variables positioned ^ ^-1 ^ run data get block ~ ~ ~ posX 1
...
execute as @e[type=area_effect_cloud,tag=CustomMissileArea,sort=nearest,limit=1] at @s if entity @s[y_rotation=-180] positioned ^ ^-1 ^ store result block ~ ~ ~ posZ int -1 run scoreboard players get Temp m.Variables
...
execute as @e[type=area_effect_cloud,tag=CustomMissileArea,sort=nearest,limit=1] at @s if entity @s[y_rotation=0] positioned ^ ^-1 ^ store result block ~ ~ ~ posZ int 1 run scoreboard players get Temp m.Variables
...
execute as @e[type=area_effect_cloud,tag=CustomMissileArea,sort=nearest,limit=1] at @s store result score @s m.tntCount run fill ^-8 ^1 ^-1 ^8 ^3 ^1 target replace tnt
