...
execute as @e[tag=mover] at @s if score temp3 Constants matches 0 run tp ~1 ~ ~17
...
execute as @e[tag=iterator] at @s if score temp3 Constants matches 0 run tp ~0 ~ ~1

scoreboard players add temp2 Constants 1

execute if score temp2 Constants <= Iterations Constants run function missilewars:custom_missile/shoot_missile/0_shoot