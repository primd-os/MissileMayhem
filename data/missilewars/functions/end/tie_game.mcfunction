scoreboard players set @e[type=area_effect_cloud,tag=BlueBase,x=0] m.GameState 3
...
execute if entity @e[type=area_effect_cloud,tag=BlueBase,x=0] run scoreboard players operation @s m.EndTimer = EndTime m.Constants