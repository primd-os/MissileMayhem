...
scoreboard players set @e[type=marker,tag=GameMarker,x=0,limit=1] m.BlackMembers 0
...
execute as @e[type=marker,tag=GameMarker,x=0,limit=1] unless score @s m.BlackLoss matches 1 if score @s m.BlackMembers < minPlayers m.Variables run scoreboard players set minTeam m.Variables 3
execute as @e[type=marker,tag=GameMarker,x=0,limit=1] unless score @s m.BlackLoss matches 1 if score @s m.BlackMembers < minPlayers m.Variables run scoreboard players operation minPlayers m.Variables = @s m.BlackMembers
