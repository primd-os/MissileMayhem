...
execute as @s[team=Black,tag=carrier] at @e[type=marker,tag=BlackBase,x=0] positioned ~ ~-20 ~-25 positioned ^ ^ ^-7 positioned ~-1 ~ ~ as @s[dx=3,dy=1,dz=50] unless entity @e[type=marker,distance=..3,tag=Flag] run tag @s add success
...
execute as @s[team=Black,tag=success] run scoreboard players add @e[type=marker,tag=GameMarker,x=0] m.BlackScore 1
...
execute as @s[tag=!carrier,team=Black,gamemode=!spectator] at @s as @e[type=marker,tag=Base,sort=nearest,limit=1] as @s[tag=!BlackBase] as @a[distance=..0.01] as @e[type=marker,distance=..1.5,tag=Flag,tag=!taken] run tag @s add claiming
