execute unless score @s BlueWin matches 1 at @e[tag=GreenBase,x=0] store success score @s PreBlueWin positioned ^-20 ^ ^ run fill ~ 0 ~ ^40 ^-4 ^ minecraft:green_terracotta replace air
execute unless score @s GreenWin matches 1 at @e[tag=BlueBase,x=0] store success score @s PreGreenWin positioned ^-20 ^ ^ run fill ~ 0 ~ ^40 ^-4 ^ minecraft:blue_terracotta replace air
