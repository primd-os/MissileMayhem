...
execute as @e[type=marker,x=0,tag=missile,tag=!Success] at @s run scoreboard players operation @s m.DirecToPlace = @a[distance=0..,scores={m.PlacedMissile=1},sort=nearest,limit=1] m.DirecToPlace

execute as @e[type=marker,x=0,tag=missile,tag=!Success] at @s if entity @a[distance=0..,scores={m.PlacedMissile=1},sort=nearest,limit=1] run function missilewars:item_managers/spawn_missile
