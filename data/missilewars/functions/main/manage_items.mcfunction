...
execute unless entity @e[tag=BlueBase,x=0,limit=1,scores={GameState=2..}] run title @s times 0 3 0
execute unless entity @e[tag=BlueBase,x=0,limit=1,scores={GameState=2..}] run title @s title {"text": ""}
execute unless entity @e[tag=BlueBase,x=0,limit=1,scores={GameState=2..}] if score @s DirectionPlace matches 1 run title @s subtitle {"text": "✥"}
execute unless entity @e[tag=BlueBase,x=0,limit=1,scores={GameState=2..}] if score @s DirectionPlace matches 0 run title @s subtitle {"text": "↑"}
execute if score @e[tag=BlueBase,x=0,limit=1] GameState matches 1 run title @s actionbar ["",{"text":"1-"},{"selector":"@e[scores={QueuePos=1},x=0]"},{"text":" 2-"},{"selector":"@e[scores={QueuePos=2},x=0]"},{"text":" 3-"},{"selector":"@e[scores={QueuePos=3},x=0]"}]
tag @s remove Active
