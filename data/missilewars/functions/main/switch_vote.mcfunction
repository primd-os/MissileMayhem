...
execute if score @s m.MapVote matches 6.. run tellraw @s {"text":"Invalid map id.","color": "red"}
execute if score @s m.MapVote matches ..-1 run scoreboard players operation @s m.MapVote = @s m.PrevMapVote
execute if score @s m.MapVote matches 6.. run scoreboard players operation @s m.MapVote = @s m.PrevMapVote
