...
execute as @e[type=marker,tag=GameMarker,x=0,scores={m.TimerTick=0}] if score @s m.Timer matches 1..2147483647 run tellraw @a[x=0] ["",{"text":"Game Starting in "},{"score":{"name":"@e[type=marker,tag=GameMarker,x=0,limit=1]","objective":"m.TimerSecs"}}]
execute as @e[type=marker,tag=GameMarker,x=0,scores={m.TimerTick=0}] if score @s m.Timer matches 1..2147483647 run playsound minecraft:block.note_block.chime block @a[x=0] ~ ~ ~ 0.5 2 1
